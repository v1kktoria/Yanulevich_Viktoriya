...
public class UserMapper {
    ...
    public UserDto toDto(User user) {
        ...
        userDto.setProperties(user.getProperties() != null ? user.getProperties().stream()
                .map(property -> modelMapper.map(property, PropertyDto.class))
                .collect(Collectors.toSet()) : new HashSet<>());

        userDto.setApplications(user.getApplications() != null ? user.getApplications().stream()
                .map(application -> modelMapper.map(application, ApplicationDto.class))
                .collect(Collectors.toSet()) : new HashSet<>());

        userDto.setReports(user.getReports() != null ? user.getReports().stream()
                .map(report -> modelMapper.map(report, ReportDto.class))
                .collect(Collectors.toSet()) : new HashSet<>());

        userDto.setReviews(user.getReviews() != null ? user.getReviews().stream()
